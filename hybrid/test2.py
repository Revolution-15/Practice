...
ADD_PLATOON_PRO_HD = 0.3
ADD_PLATOON_PRO_AV = 0.3
...
def add_single_platoon_HD(step, lane):

    vid = "v.%d.%s.%d.h" %(step/ADD_PLATOON_STEP, lane[0], int(lane[2]))
    num = return_routeID(lane)
    #print(lane)
    routeID = "route_" + num  
    dl = lanes(lane[0])
...
def add_platoons(step):
    for lane in LANE_NUM:    # lane a,b,c,d 
            if random.random() <= ADD_PLATOON_PRO_AV:
                add_single_platoon(step, lane)
            if random.random() <= ADD_PLATOON_PRO_HD:
                add_single_platoon_HD(step, lane)
...
def my_alg_01(new,a):
    ...
    if second_output == third_output:
        ...
        return third_output
########################################################################
def my_alg_02(new,a,b):
    print("Two human wala")
    human = a
    human2 = b

    for i in new:
        if i!=0 and i!=human and i!=human2:
            if i in conflict_matrix[human] or i in conflict_matrix[human2]:
                ind = new.index(i)
                new[ind]=0
    ...
    second_list = []
    for i in first_output:
        if i!=first_output[z] and i!=first_output[zt] and i!=0:
            temp2 = [first_output[0], first_output[1], first_output[2], first_output[3]]

            for j in temp2:
                if i!=j and j!=z and j!=zt and j!=0:
                    if j in conflict_matrix[i]:
                        ind = temp2.index(j)
                        temp2[ind] = 0
            second_list.append(temp2)
        if i == 0 or i == first_output[z] or first_output[zt]:
            second_list.append(all_zeros)
    #print(second_list)
    ...
    if second_output == third_output:
        ...
        return third_output
########################################################################
def my_alg_03(new,a,b,c):
    print("three human wala")
    human = a
    human2 = b
    human3 = c
    ...
    while step < 360000:  # 1 hour       
        ...
        if c ==0:
            ...
            for i in action_list:
                if traci.vehicle.getColor(i) == (255,0,0,255):
                    hlist.append(i)
                    human += 1

            if human == 1:
                h1=hlist[0]
                h1 = hlist[0].split(".")[2] + "_" + hlist[0].split(".")[3]
                output_list = my_alg_01(input_list,h1)


            if human == 2:
                h1=hlist[0].split(".")[2] + "_" + hlist[0].split(".")[3]
                h2=hlist[1].split(".")[2] + "_" + hlist[1].split(".")[3]
                output_list = my_alg_02(input_list,h1,h2)


            if human == 3:
                h1=hlist[0].split(".")[2] + "_" + hlist[0].split(".")[3]
                h2=hlist[1].split(".")[2] + "_" + hlist[1].split(".")[3]
                h3=hlist[2].split(".")[2] + "_" + hlist[2].split(".")[3]
                output_list = my_alg_03(input_list,h1,h2,h3)

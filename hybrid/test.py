...
def main():
    ...
    while step < 360000:  # 1 hour       
        ...
        input_list = conversion(action_list)
        ...
        print('input_vehicles:',input_list,'               ' ,'output_vehicles:',output_list)
 
        if action_list is not []:
            for veh in action_list:
                allowed_veh = veh.split(".")[2]+'_'+veh.split(".")[3]
                if allowed_veh in output_list:
                    traci.vehicle.setSpeed(veh, 10.0)
                else:
                    traci.vehicle.setSpeed(veh, 0.0)
